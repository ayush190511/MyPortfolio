...
function initPortfolio() {
  ...
  // 13. GEMINI-STYLE AMBIENT CURSOR AURA GLOW (Time-Delayed Elastic Pursuit)
  const cursorGlow = document.getElementById('cursorGlowAmbient');
  if (cursorGlow && window.matchMedia('(pointer: fine)').matches && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    ...
    // Time-Buffer History Queue (Stores timestamped coordinates)
    const positionHistory = [];
    const DELAY_MS = 220; // 220ms perceptible cinematic chase delay

    const onMouseMove = (e) => {
      const now = performance.now();
      positionHistory.push({ x: e.clientX, y: e.clientY, time: now });
    ...
    const renderGlow = (now) => {
      if (hasEntered) {
        ...
        // Prune old history and find the delayed coordinate
        while (positionHistory.length > 1 && positionHistory[1].time <= delayedTime) {
        ...
        // Fluid spring physics towards the delayed target position
        ...
        // Velocity-based scale expansion during active chase
        const speed = Math.sqrt(vx * vx + vy * vy);
        const targetScale = Math.min(1.22, 1.0 + speed * 0.01);
        currentScale += (targetScale - currentScale) * 0.14;

        cursorGlow.style.transform = `translate3d(${currentX.toFixed(2)}px, ${currentY.toFixed(2)}px, 0) scale(${currentScale.toFixed(3)})`;
      }
